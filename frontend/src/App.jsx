...
import { fetchPersonalizedMenuIdeas } from "./services/menuAnalysis";
import "./App.css";
...
function App() {
  ...
  const [modal, setModal] = useState(() => localStorage.getItem("kfood-session") === "active" ? "" : "welcome");
  ...
  const recommendedMenus = useMemo(() => {
    if (aiMenuIdeas?.menus?.length) return aiMenuIdeas.menus.slice(0, 5);
  ...
  useEffect(() => { let active = true; fetchPersonalizedMenuIdeas(profile).then((result) => { if (active) setAiMenuIdeas(result); }).catch(() => { if (active) setAiMenuIdeas(null); }); return () => { active = false; }; }, [profile]);
