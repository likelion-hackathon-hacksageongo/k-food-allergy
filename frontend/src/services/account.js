...
export async function authenticate(mode, email, password) {
  const path = mode === "signup" ? "/api/accounts/register/" : "/api/accounts/login/";
  const body = mode === "signup"
    ? { email, password, password_confirm: password }
    : { email, password };
  return request(path, { method: "POST", headers: API_HEADERS, body: JSON.stringify(body) });
}
...
export async function loadProfile() {
  ...
  return request("/api/profiles/me/", { headers: { Authorization: `Bearer ${token}` } });
}
