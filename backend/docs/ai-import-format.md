...
[
  {
    "restaurant": "Hongdae Sundubu House",
    "menu_item": "Seafood Sundubu Jjigae",
...
| 필드 | 값 | 설명 |
|---|---|---|
| `restaurant` | 문자열 | DB에 이미 있는 식당 이름과 **정확히 일치**해야 함 |
| `menu_item` | 문자열 | 그 식당의 메뉴명과 **정확히 일치**해야 함 |
...
- `restaurant` / `menu_item`은 DB에 이미 존재해야 매칭됩니다 (Data팀이 먼저 식당/메뉴를 등록해둔 상태여야 함). 없으면 해당 항목은 건너뛰고 결과에 로그로 표시됩니다.
